...
import uz.ibrohim.sovchiuz.databinding.FragmentHumoBinding

class HumoFragment : Fragment() {

    private var _binding: FragmentHumoBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentHumoBinding.inflate(inflater, container, false)

        val sharedPreferences: SharedPreferences =
            requireActivity().getSharedPreferences("user_card", Context.MODE_PRIVATE)

        binding.humoNumber.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(p0: Editable?) {
            }
            override fun beforeTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
            }
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                when (binding.humoNumber.text.toString()) {
                    "8600" -> {
        ...
        binding.humoUzcard.setOnClickListener {
            replaceFragment(UzcardFragment())
        }

        binding.humoVisa.setOnClickListener {
            replaceFragment(VisaFragment())
        }

        binding.humoBtn.setOnClickListener {
            val number = binding.humoNumber.text.toString()
            val year = binding.humoYear.text.toString()
